...
simul.bull<-function(nfemales=10,lambda.nbulls=12.5,first.day=20170524,mu.days=3.9,theta.days=2.32,
                     p.isolated.bull=0.2,rest.days=4,mean.hour=82800, sd.hour=3600, nplaces=2,
                     volley.shape=1.0492316876,volley.rate=0.0014073256,dispersal=1){
  #Draw a random number of splashes for each female
  nbulls<-rpois(nfemales,lambda.nbulls)
  #Draw a random starting date from a negative binomial
  start.spawn<-ymd(first.day)+days(rnegbin(nfemales,mu=mu.days,theta=theta.days)) 
  central.place<-round(runif(nfemales,min=1,max=nplaces))
  #full.data= data.frame(matrix(data=NA, nrow=sum(nbulls), ncol=5, dimnames=list(c(), c("place.bulls","date.bulls","hour.bulls","time.bulls","std.time.bulls"))),stringsAsFactors=F)
  place.bulls<-numeric(length=sum(nbulls))
  date.bulls<-ymd(rep('2017-05-27',length=sum(nbulls)))
  hour.bulls<-hms(rep('10:00:00',length=sum(nbulls)))
  for(f in 1:nfemales){
    ...
    date.bulls[(cumsum(nbulls)[f]-nbulls[f]+1):cumsum(nbulls)[f]]<-start.spawn[f]+days(cumsum(ifelse(rbinom(nbulls[f],1,p.isolated.bull),rpois(nbulls[f],lambda=rest.days),0)))  
    # The hour of the first splash of the night is drawn from the Cassou-Leins Normal distribution
    hour.bulls[(cumsum(nbulls)[f]-nbulls[f]+1):cumsum(nbulls)[f]]<-seconds(round(rnorm(nbulls[f],mean.hour,sd.hour)))
    # The place of the first splash of the night is drawn from a truncated Normal centered around a central place
    place.bulls[(cumsum(nbulls)[f]-nbulls[f]+1):cumsum(nbulls)[f]]<-round(rtnorm(nbulls[f],mean=central.place[f],sd=dispersal,lower=1,upper=nplaces))
    ...
    # All splashes of a volley occur in the same place
     for(i in (cumsum(nbulls)[f]-nbulls[f]+2):cumsum(nbulls)[f]){
  ...
  full.data<-data.frame(place.bulls,date.bulls,hour.bulls,time.bulls,std.time.bulls)
  return(full.data)
}
############################################################################################
...
### MAIN CODE ####
#######################
sampling.plan<-read.csv("G:/Mes Documents/St_Pee/Alose/Simulations/fictional sampling2.csv",header=T,sep=";")
...
for(r in 1:20){
 ...
 observed.nbulls[r]<-nrow(observed.data) #total number of splashes observed in the season
 max.in.1.night[r]<-max(tabulate(yday(observed.data$time.bulls+hours(12)))) #maximum number of splashes observed in a single night
}
windows()
plot(as.data.frame(cbind(actual.nfemales,actual.nbulls,observed.nbulls,max.in.1.night)))
